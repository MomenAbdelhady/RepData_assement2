...
g <- g + geom_point(colour="#56B4E9") + geom_text(size=3, hjust=1, vjust=1, angle=-45) + scale_y_log10(limits=c(1,10000)) + scale_x_log10(limits=c(10,100000))
g + scale_size(range=c(2,40)) + theme(legend.position="none")
```

```{r,echo=TRUE}
library(reshape2)
storm.harmful.melt <- melt(storm.harmful, id=c("EVTYPE"), measure=c("total.injuries","total.fatalities"))
